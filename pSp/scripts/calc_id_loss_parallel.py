...
def extract_on_paths(file_paths):
	...
	for res_path, gt_path in file_paths:
		...
		if True:
			...
			score = float(input_id.dot(result_id))   # actually this script calculates identity similarity
...
def run(args):
	...
	for f in os.listdir(args.data_path):
		...
		if f.endswith(".jpg") or f.endswith('.png'):
			file_paths.append([image_path, gt_path.replace('.png','.png')])   # modified
